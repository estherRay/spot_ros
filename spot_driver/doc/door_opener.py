...
from spot_msgs.srv import SpotPositionRequest
...
def check_estop(self):
    ...
    Args:
        robot: (Robot) Interface to Spot robot.
    """
    assert not self.is_estopped(), "Robot is estopped. Please use an external E-Stop client, " \
...
def walk_to_object_in_image(self, request_manager, debug):
    ...
    """
    manip_client = self.ensure_client(ManipulationApiClient.default_service_name)
...
def open_door(self, request_manager, snapshot):
    ...
    door_client = self.ensure_client(DoorClient.default_service_name)
...
def execute_open_door(self, spot_wrapper):
...
#    stand(robot)
    ...
    image_dict = get_images_as_cv2(self, sources)
    ...
    manipulation_feedback = walk_to_object_in_image(self, request_manager, debug=False)
    time.sleep(3.0)
    ...
    open_door(self, request_manager, snapshot)
...
def open_door_main(self, spot_wrapper):
...
#    lease = lease_client.take()
    try:
        ...
        execute_open_door(self, spot_wrapper)
        comment = "Opened door successfully."
        self.operator_comment(comment)
